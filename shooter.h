...
public:
    ...
    void update(const GameInputState& input) {
        ...
        if (currentState == STATE_PLAYING) {
            ...
            bool dirChanged = (input.up != lastInput.up) || (input.down != lastInput.down) ||
                ...
                for (int i = 0; i < MAX_BULLETS; i++) {
                    if (!bullets[i].active) {
                        ...
                        switch (playerDir) {
                            ...
                            case DIR_UP_RIGHT:   bullets[i].dx =  1; bullets[i].dy = -1; break;
                            case DIR_RIGHT:      bullets[i].dx =  1; bullets[i].dy =  0; break;
                            case DIR_DOWN_RIGHT: bullets[i].dx =  1; bullets[i].dy =  1; break;
                            case DIR_DOWN:       bullets[i].dx =  0; bullets[i].dy =  1; break;
                            case DIR_DOWN_LEFT:  bullets[i].dx = -1; bullets[i].dy =  1; break;
                            case DIR_LEFT:       bullets[i].dx = -1; bullets[i].dy =  0; break;
                            case DIR_UP_LEFT:    bullets[i].dx = -1; bullets[i].dy = -1; break;
                        }
        ...
        if (now - lastBulletTime >= BULLET_INTERVAL_MS) {
            for (int i = 0; i < MAX_BULLETS; i++) {
                if (bullets[i].active) {
                    ...
                    if (bullets[i].x < 0 || bullets[i].x >= PANEL_WIDTH || 
                        bullets[i].y < 0 || bullets[i].y >= PANEL_HEIGHT) {
        ...
        if (now - lastEnemyMoveTime >= ENEMY_MOVE_INTERVAL_MS) {
            for (int i = 0; i < NUM_ENEMIES; i++) {
                if (enemies[i].active) {
                    ...
                    if (enemies[i].x < -4 || enemies[i].x >= PANEL_WIDTH + 4 || 
                        enemies[i].y < -4 || enemies[i].y >= PANEL_HEIGHT + 4) {
                        spawnEnemy(i);
                    }
                }
            }
            ...
            for (int i = 0; i < MAX_PARTICLES; i++) {
                if (particles[i].active) {
                    ...
                    if (particles[i].life <= 0 || 
                        particles[i].x < 0 || particles[i].x >= PANEL_WIDTH || 
                        particles[i].y < 0 || particles[i].y >= PANEL_HEIGHT) {
